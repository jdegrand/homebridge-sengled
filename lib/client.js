...
initializeMqtt() {
	...
	this.mqttClient.on("connect", () => {
		me.log("MQTT client connected:", options.clientId);
	})
}
			...
			.then((response) => {
				if (response.data.ret == 100) {
					...
					let homekitDevices = devices.map((device) => {
						...
						const brightnessContextData = new BrightnessContextData(`${Math.floor(parseInt(brightnessValue) * 255 / 100)}`);
  ...
  deviceSetBrightness(deviceId, brightness) {
    return new Promise((fulfill, reject) => {
		const payload = {
			dn: deviceId,
			type: 'brightness',
			value: `${Math.ceil(brightness * 100 / 255)}`,
			time: new Date().getTime(),
		};
